...
Creates a new csv for each game in a folder

Author: Demarco Guajardo, William Bingham
'''
...
def contains_valid_aub_tig(file_path):
    try:
        ...
        for row in data:
            if row.get("PitcherTeam") == "AUB_TIG":
                if row.get("PitcherTeam") == "AUB_PRC" or row.get("BatterTeam") == "AUB_PRC":
                    return False  # Disqualify this file
...
def process_csv(file_path, output_dir):
    try:
        ...
        if data is not None:
            ...
            game_id = data[0].get('GameID', "unknown_game")
            output_file = os.path.join(output_dir, f"{game_id}_player_stats.csv")
...
if __name__ == "__main__":
    input_dir = input("Enter the path to the directory containing CSV files: ")
    output_dir = input("Enter the path to the directory where you want the output files saved: ")
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.lower().endswith(".csv"):
                ...
                try:
                    if contains_valid_aub_tig(file_path):
                        process_csv(file_path, output_dir)
                    else:
                        print(f"Skipping {file_path} (missing AUB_TIG or contains AUB_PRC)")
                except Exception as e:
